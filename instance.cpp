...
MoveRec::MoveRec(const QString& fen, PieceColor color, int rowcols, const Record& record)
    : fen { fen }
    , color { color }
    , rowcols { rowcols }
    , record { record }
...
{
    PSeat fseat { board_->getSeat(frow, fcol) }, tseat { board_->getSeat(trow, tcol) };
    return appendMove_seats__(fseat, tseat, remark, isOther);
}
...
Instance::PMove Instance::appendMove_iccszh(QString iccszhStr, RecFormat fmt, const QString& remark, bool isOther)
{
    if (fmt != RecFormat::PGN_ICCS) {
        if (isOther)
            curMove_->undo();
        QPair<PSeat, PSeat> seats = board_->fromToSeats(iccszhStr);
        assert(seats.first && seats.second);
        if (isOther)
            curMove_->done();
        return appendMove_seats__(seats.first, seats.second, remark, isOther);
    }
...
Instance::PMove Instance::appendMove_zh_tolerateError(QString zhStr, bool isOther)
{
    ...
    if (!seats.first)
        return Q_NULLPTR;
    return appendMove_seats__(seats.first, seats.second, "", isOther);
}
...
void Instance::changeSide(ChangeType ct)
{
    ...
    PSeat firstSeat { rootMove_->next_ ? rootMove_->next_->fseat_ : nullptr };
    setFEN__(board_->getFEN(), firstSeat ? firstSeat->piece()->color() : PieceColor::RED);
    ...
    std::function<void(const PMove&)>
        __appendMoveRec = [&](const PMove& move) {
            ...
            moveReces.append(MoveRec(board_->getFEN(),
                move->fseat_->piece()->color(), move->rowcols(), Record(0, false, false, false, false)));
...
bool Instance::readXQF__(const QString& fileName)
{
    ...
    std::function<void(bool)> __readMove = [&](bool isOther) {
        ...
        PSeat fseat { curMove_->fseat_ }, tseat { curMove_->tseat_ };
        if (fseat && tseat
            && fseat->row() == frow && fseat->col() == fcol
            && tseat->row() == trow && tseat->col() == tcol) {
            if (!remark.isEmpty())
...
bool Instance::readBIN__(const QString& fileName)
{
    ...
    std::function<void(bool)> __readMove = [&](bool isOther) {
        ...
        QString remark {};
        stream.readRawData(&frowcol, 1);
        stream.readRawData(&trowcol, 1);
        stream.readRawData(&tag, 1);
        if (tag & 0x20)
            stream >> remark;
        appendMove_rc(frowcol / 10, frowcol % 10, trowcol / 10, trowcol % 10, remark, isOther);
...
bool Instance::writeBIN__(const QString& fileName) const
{
    ...
    std::function<void(const PMove&)> __writeMove = [&](const PMove& move) {
        char tag = ((move->next_ ? 0x80 : 0x00)
             ...
             frowcol = move->frowcol(),
             trowcol = move->trowcol();
        stream.writeRawData(&frowcol, 1);
        stream.writeRawData(&trowcol, 1);
    ...
    std::function<void(bool, QJsonObject)>
        __readMove = [&](bool isOther, QJsonObject item) {
            ...
            if (!mitem.isUndefined()) {
                ...
                int pos0 { mvstr.indexOf(' ') }, pos1 { mvstr.indexOf(' ', pos0 + 1) },
                    frowcol { mvstr.left(pos0).toInt() }, trowcol { mvstr.mid(pos0 + 1, pos1 - pos0).toInt() };
                QString remark { mvstr.mid(pos1 + 1) };

                // 数组方式
                //QJsonArray ftrowcol_rem = mitem.toArray();
                //int frowcol { ftrowcol_rem[0].toInt() }, trowcol { ftrowcol_rem[1].toInt() };
                //QString remark = ftrowcol_rem.size() < 3 ? "" : ftrowcol_rem[2].toString();
                appendMove_rc(frowcol / 10, frowcol % 10, trowcol / 10, trowcol % 10, remark, isOther);
            }
...
bool Instance::writeJSON__(const QString& fileName) const
{
    ...
    std::function<QJsonObject(const PMove&)>
        __getJsonMove = [&](const PMove& move) {
            ...
            if (move != rootMove_) {
                QString mvstr;
                QTextStream(&mvstr) << move->frowcol() << ' ' << move->trowcol() << ' ' << move->remark_;
                /*
                // 数组方式
                QJsonArray mitem { move->frowcol(), move->trowcol() };
                if (!move->remark_.isEmpty())
                    mitem.append(move->remark_);
                item.insert("m", mitem);
                //*/
                item.insert("m", mvstr);
            }
...
Instance::PMove Instance::appendMove_seats__(PSeat& fseat, PSeat& tseat, const QString& remark, bool isOther)
{
    assert(fseat && tseat);
    if (isOther) {
        curMove_->undo();
    ...
    bool can { board_->isCanMove(fseat, tseat) };
    QString zhStr {};
    if (can)
        zhStr = board_->pgn_zhStr(fseat, tseat);
    /*/ 观察棋盘局面与着法
    Tools::writeTxtFile("test.txt",
    ...
    PMove move = curMove_->appendMove(fseat, tseat, zhStr, remark, isOther);
    isOther ? goOther() : go();
    return move;
}
...
void Instance::setFEN__(const QString& fen, PieceColor color)
{
    info_["FEN"] = (fen + " "
        + (color == PieceColor::RED ? "r" : "b") + " - - 0 1");
}
...
int Instance::Move::rowcols() const { return getRowcols(frowcol(), trowcol()); }

QString Instance::Move::iccs() const
{
    return ((fseat_ && tseat_)
            ? (PieceManager::getColICCSChar(fseat_->col()) + QString::number(fseat_->row())
                + PieceManager::getColICCSChar(tseat_->col()) + QString::number(tseat_->row()))
            : QString {});
}

Instance::PMove Instance::Move::appendMove(PSeat fseat, PSeat tseat, const QString& zhStr, const QString& remark, bool isOther)
{
...
{
    fseat_ = board->getOtherSeat(fseat_, ct);
    tseat_ = board->getOtherSeat(tseat_, ct);
    zhStr_ = board->pgn_zhStr(fseat_, tseat_);
...
QString Instance::Move::toString() const
{
    ...
    if (fseat_ && tseat_) {
        QTextStream stream(&qstr);
        stream << frowcol() << '_' << trowcol() << '-' << iccs() << ':' << zhStr_ << '{' << remark_ << "}\n";
    }
    return qstr;
}
...
/*
static void writeFileNames__(const QString& fileName, void* arg)
{
